...
- **Primary visual: `yoshi389111/github-profile-3d-contrib@v0.9.3` with our own electric theme** (`conf/github-profile-3d-contrib.json`) — the year as a 3D isometric skyline on Void `#08080D`, towers ramping `#171C33 → #2B3B9E → #3A52FF → #5B8CFF → #7FD8FF`, violet radar, cyan totals. The stock `night-rainbow` was retired: its red→yellow half-spectrum fought the palette and tired the eyes. Upstream hardcodes GitHub language colors in the pie (orange HTML, yellow JS), so `scripts/recolor-graph-langs.mjs` remaps every inline fill onto the iridescence ramp post-render — the SVG's only inline fills are the pie's (v0.9.3, verified), towers/radar are class-styled. Published as `output/contribution-3d.svg`; ships its own date-range stamp, dark on both GitHub themes like the hero.
